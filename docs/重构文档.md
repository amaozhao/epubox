...
```
engine/item/
├── placeholder.py      # 占位符核心
├── renumberer.py       # 新增：全局→局部索引转换
...
#### PlaceholderRenumberer 类

```python
# engine/item/renumberer.py
...
async def translate_step(step_input: StepInput) -> StepOutput:
    ...
    for attempt in range(MAX_TRANSLATION_RETRIES):
        translated = await call_translator(chunk.original, local_tag_map)

        # 验证占位符
        is_valid, _ = validate_placeholders(translated, local_tag_map)
        if is_valid:
            chunk.translated = placeholder_mgr.restore_to_global(
                translated, chunk.global_indices
            )
            chunk.status = TranslationStatus.TRANSLATED
            return StepOutput(content=chunk)
...
async def token_alignment_fallback(original, local_tag_map, global_indices):
    """Phase 2: 纯净文本翻译 + 重新插入占位符"""
    clean = _remove_placeholders(original, list(local_tag_map.keys()))
    if not clean.strip():
        return None

    translated = await translate_clean_text(clean)
    aligned = _align(original, translated, list(local_tag_map.keys()))

    if not validate_placeholders(aligned, local_tag_map):
        return None

    return PlaceholderManager.restore_to_global(aligned, global_indices)


async def translate_clean_text(clean_text: str) -> str:
    """翻译纯净文本（Phase 2 专用，不带占位符）"""
    translator = get_translator()
    response = await translator.arun(clean_text)
    if isinstance(response.content, TranslationResponse):
        return response.content.translation
    raise ValueError("翻译响应格式错误")
...
def _find_positions(text: str, placeholders: List[str]) -> List[Tuple[int, int, str]]:
    ...
    for ph in placeholders:
        ...
        if idx != -1:
            ...
            remaining, offset = remaining[idx + len(ph):], offset + idx + len(ph)
    return positions
...
async def translate_step_final_fallback(chunk, placeholder_mgr) -> StepOutput:
    """Phase 3: 返回原文（保留结构完整性）"""
    chunk.translated = placeholder_mgr.restore_to_global(
        chunk.original, chunk.global_indices
    )
    chunk.status = TranslationStatus.UNTRANSLATED
    # success=True 让 workflow 继续（proofread 会优雅处理未翻译状态）
    return StepOutput(content=chunk, success=True)
```
...
def validate_placeholders(text: str, tag_map: Dict[str, str]) -> Tuple[bool, str]:
    ...
    if len(found) != len(tag_map):
        return False, f"数量: 期望{len(tag_map)}, 实际{len(found)}"

    # 2. 检查顺序
    indices = sorted([int(x) for x in found])
    expected = list(range(len(tag_map)))
    if indices != expected:
        return False, f"顺序: 期望{expected}, 实际{indices}"
...
**新流程：先按 HTML 结构分割，再做占位符替换**，保证句子完整性。

```
原流程：
HTML → TagPreserver → [id0]... → 在占位符位置分割 → 可能句子中间切断

新流程：
HTML → BeautifulSoup 解析 → PreCodeExtractor → 按块级标签分割
  ↓
如果超限 → 在句子边界分割
  ↓
对每个 raw_chunk → TagPreserver.preserve_tags() → [id0]...
```
...
| 分类 | 标签 | 说明 |
|-----|------|------|
| **安全分割点** | `</p>`, `</div>`, `</h1>`-`</h6>`, `</blockquote>`, `</section>`, `</article>`, `</header>`, `</footer>`, `</main>`, `</aside>`, `</figure>`, `</figcaption>` | 块级边界，可以安全分割 |
| **容器级分割点** | `</table>`, `</ul>`, `</ol>` | 万不得已时分割，包含完整结构 |
| **禁止分割点** | `</tr>`, `</td>`, `</th>`, `</li>` | 会破坏表格/列表结构 |

**示例**：
```html
<table>
  <tr><td>A</td><td>B</td></tr>  ← 不能在这里分割
  <tr><td>C</td><td>D</td></tr>  ← 不能在这里分割
</table>                              ← 可以在</table>后分割
```

#### 导航文件（toc.ncx / nav.xhtml）

| 分类 | 标签 | 说明 |
|-----|------|------|
| **安全分割点** | `</navPoint>`, `</p>`（如果有） | 保持 navPoint 完整 |
| **禁止分割点** | `</navLabel>`, `</content>`, `</navMap>` | 会破坏导航结构 |
...
**Level 1 - 块级边界（优先）**：
- 分割点：`</p>`, `</div>`, `</h1>`-`</h6>`, `</blockquote>` 等
- 优先级：最高

**Level 2 - 句子边界（当 Level 1 无法满足 token 限制时）**：
- 分割点：`.` `!` `?` `。` `！` `？` 后接大写字母/汉字/HTML标签
- 优先级：中

**Level 3 - 容器级结束标签（当 Level 1+2 都无法满足时）**：
- 分割点：`</table>`, `</ul>`, `</ol>`
- 优先级：低
- 说明：整个表格/列表作为完整 chunk

**Level 4 - 硬切（极少情况）**：
- 当以上都无法满足时，按 token_limit 硬切
- 警告：这是最后手段

### 分割算法

```python
def chunk_by_html_tags(self, html: str, token_limit: int = 1500, is_nav_file: bool = False) -> List[str]:
    """
    统一分割接口
...
def _chunk_normal_html(self, html: str, token_limit: int) -> List[str]:
    ...
    split_positions = set()
    for tag in SAFE_BLOCK_END_TAGS:
...
```
- 分割点：`</p>` 后
- 结果：`["<p>Sentence one.</p>", "<p>Sentence two.</p>"]`

**示例 2：超长表格**
```html
<table><tr><td>A</td></tr><tr><td>B</td></tr></table>
```
- 分割点：`</table>` 后（没有 `</tr>` 什么事）
- 结果：`["<table>...全部表格...</table>"]`
...
| 保证 | 说明 |
|-----|------|
| **块级标签不被切断** | 分割点在 `</p>` 等后面，不会切断开始标签 |
| **表格/列表结构完整** | `</tr>`, `</li>` 不可分割 |
| **句子完整性优先** | 超长时优先在句子边界分割 |
| **navPoint 结构完整** | 导航文件的 `<navPoint>...</navPoint>` 不会被切断 |
| **保底机制** | 找不到句子边界时按 token 硬切 |
...
```python
class PlaceholderManager:
    def create_placeholder(self, tag: str) -> str: "[id0]"
    def get_local_tag_map(self, indices: List[int]) -> Dict[str, str]
    def restore_to_global(self, text: str, indices: List[int]) -> str
```
...
```python
class Renumberer:
    def renumber(self, text: str, tag_map: Dict, offset: int) -> Dict:
        # 返回 {'text': '[id0]Hello[id1]', 'tag_map': {...}, 'indices': [5, 6]}
```
...
```python
class TagPreserver:
    def preserve(self, html: str) -> Tuple[str, PlaceholderManager]
```
...
```python
class HtmlChunker:
    def chunk(self, text: str, tag_map: Dict, is_nav: bool = False) -> List[Chunk]
```
