...
class TestDatabaseModels:
    ...
    @pytest.mark.asyncio
    async def test_create_oauth_account(self, db_session: AsyncSession):
        """测试创建 OAuth 账号"""
        # 创建用户
        user = User(
    ...
    @pytest.mark.asyncio
    async def test_user_oauth_relationship(self, db_session: AsyncSession):
        """测试用户和 OAuth 账号的关系"""
        # 创建用户
        user = User(
    ...
    async def test_cascade_delete(self, db_session: AsyncSession):
        ...
        # 删除用户，验证 OAuth 账号也被级联删除
        await db_session.delete(user)
        await db_session.commit()

        # 验证用户和 OAuth 账号都已被删除
        user_query = select(User).where(User.id == user.id)
        oauth_query = select(OAuthAccount).where(OAuthAccount.id == oauth_account.id)

        user_result = await db_session.execute(user_query)
        oauth_result = await db_session.execute(oauth_query)

        assert user_result.scalar_one_or_none() is None
        assert oauth_result.scalar_one_or_none() is None
