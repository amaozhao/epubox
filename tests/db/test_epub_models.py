...
@pytest.mark.asyncio
async def test_translation_progress_creation():
    """Test creating a translation progress record."""
    # 准备测试数据
    book_id = "test-book-1"
    total_chapters = {
        "chapter1.xhtml": {"id": "chapter1.xhtml", "type": "html", "name": "Chapter 1"},
        "chapter2.xhtml": {"id": "chapter2.xhtml", "type": "html", "name": "Chapter 2"},
    }

    # 创建记录
    progress = TranslationProgress(
        book_id=book_id, total_chapters=total_chapters, status=TranslationStatus.PENDING
    )
...
@pytest.mark.asyncio
async def test_translation_progress_update():
...
@pytest.mark.asyncio
async def test_translation_progress_completion():
...
@pytest.mark.asyncio
async def test_translation_progress_validation():
    ...
    with pytest.raises(ValueError):
        TranslationProgress(
            book_id="test-book-5",
            total_chapters={"chapter1.xhtml": {"id": "chapter1.xhtml", "type": "html"}},
...
@pytest.mark.asyncio
async def test_translation_progress_calculation():
...
@pytest.mark.asyncio
async def test_translation_progress_status_transition():
