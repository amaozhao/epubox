...
class TestProgressManager:
    """Test suite for ProgressManager."""
    ...
    async def engine(self):
        ...
        engine = create_async_engine(self.TEST_DATABASE_URL)
    ...
    @pytest.fixture
    async def session_maker(self, engine):
        """Create a test session maker."""
        async_session = async_sessionmaker(engine, expire_on_commit=False)
        return async_session

    @pytest.fixture
    async def test_session(self, session_maker):
    ...
    @pytest.fixture
    def manager(self, monkeypatch, session_maker):
    ...
    async def test_create_progress(self, manager, test_book_id, test_chapters):
    ...
    async def test_get_progress_nonexistent(self, manager, test_book_id):
        """Test getting progress for a nonexistent book."""
        progress = await manager.get_progress(test_book_id)
        assert progress is None

    async def test_get_progress_existing(self, manager, test_book_id, test_chapters):
    ...
    async def test_update_chapter(self, manager, test_book_id, test_chapters):
    ...
    async def test_complete_translation_flow(
        self, manager, test_book_id, test_chapters
    ):
    ...
    async def test_progress_percentage(self, manager, test_book_id, test_chapters):
